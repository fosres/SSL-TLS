...
void get_params(char const * uri,char ** hostname,char ** pathname)
{
	...
	strncat_s(url,MAX_BUFFER_SIZE,uri,MAX_BUFFER_SIZE-strlen(url)-1);


	if ( strstr(url,"//") != NULL )
	{ 
		strncat_s(*hostname,MAX_BUFFER_SIZE,&url[abs(strstr(url,"//")-&url[0]) + 2],MAX_BUFFER_SIZE - strlen(*hostname)-1);
	...
	{
		strncat_s(*hostname,MAX_BUFFER_SIZE,&url[0],MAX_BUFFER_SIZE-strlen(*hostname)-1);
	...
	{	
		...
		strncat_s(*pathname,MAX_BUFFER_SIZE,&url[abs( strstr(*hostname,"/")  - &url[0] ) + 1],MAX_BUFFER_SIZE-strlen(*pathname)-1);
	}
...
int main(int argc, char ** argv)
{
	...
	snprintf_s(test,TEST_LEN,"GET /%s HTTP/1.1\r\n\0",path);
	...
	snprintf_s(test,TEST_LEN,"Host: %s\r\n\0",host);
	...
	snprintf_s(test,TEST_LEN,"Connection: close\r\n\r\n\0");
		...
		&& (msg[0] != '\0') )
	{
		...
		memset_s(msg,MAX_BUFFER_SIZE,0,sizeof(msg));	
	}
